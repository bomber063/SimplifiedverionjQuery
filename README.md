...
等等  
4. 并且此时还用到了**闭包**，闭包就是一个函数（比如addClass）用到的该函数外面的变量（比如let node），那么这个函数和函数外面的变量统称为闭包。

#### 闭包的好处
这个外面的变量，在构造函数（比如jQuery）内部（包括内部的API）一直在用它，但是用户(也就是构造函数外部)却访问不到。注意这里的访问不到不是绝对访问不到，而是需要加上索引，比如node2修改为node2[0],这个node2[0]其实就是一个div元素，你就可以用原生的JavaScript来操作这个DOM元素啦,因为这个变量（比如let node）的作用域在JQuery这个函数里面，相当于一个隐藏的或者中间变量。
```
...
### 设置多个文本和以数组方式获取多个文本的结合，就可以合并前面两个代码了
其实只需要用if..else即可
部分代码如下
```
...
```
## 真实的jQuery.text()
[jQuery.text](https://www.jquery123.com/text/)当他括号里面为空有参数的时候，是设置多个文本，但是，当括号里面为空的时候，它返回的是一个字符串，并且使把所获得的文本全部都放在一起的一个字符串。  
[英文版jQuery.text](https://api.jquery.com/text/)  
